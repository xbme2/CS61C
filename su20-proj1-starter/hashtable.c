...
void insertData(HashTable *table, void *key, void *data) {
  ...
  int hashkey = table->hashFunction(key);
  if(!table->data[hashkey]){
    table->data[hashkey] = malloc(sizeof(struct HashBucket ));
    table->data[hashkey]->key = key;
    table->data[hashkey]->data = data;
    table->data[hashkey]->next = NULL;
  }
  else{
    struct HashBucket *temp = table->data[hashkey];
    while(temp->next) temp = temp->next;
    struct HashBucket *data = malloc(sizeof(struct HashBucket ));
    data->key = key;
    data->data = data;
    data->next = NULL;
    temp->next = data;
...
void *findData(HashTable *table, void *key) {
  ...
  int hashkey = table->hashFunction(key);
  if(!table->data[hashkey]) return NULL;
  struct HashBucket *temp = table->data[hashkey];
  while(temp){
    if(table->equalFunction(temp->key,key) !=0 ) return temp->data;
    temp = temp->next;
  }
  return NULL;
